...
const DB = (() => {
    ...
    function resetUser() { _prefix = 'rabix_default_'; }
    function _key(k) { return _prefix + k; }
    const K = { REGISTRO: 'registro', OVERRIDES: 'overrides', SYNC_QUEUE: 'pending_sync_queue', SYNC_CURSOR: 'sync_cursor' };
    const SYNC_ENDPOINT_KEY = 'rabix_sync_endpoint';
    ...
    function _enqueueSync(type, payload) {
        const q = _get(K.SYNC_QUEUE);
        q.push({
    ...
    function saveOverride(fecha, escenario) {
        ...
        ov[fecha] = escenario;
        _set(K.OVERRIDES, ov);
        _enqueueSync('saveOverride', { fecha, escenario });
    }
    ...
    function getSyncQueue() { return _get(K.SYNC_QUEUE); }
    function getSyncEndpoint() { return localStorage.getItem(SYNC_ENDPOINT_KEY) || ''; }
    ...
    async function flushSyncQueue() {
        ...
        if (!endpoint || typeof fetch !== 'function') return { ok: false, reason: 'missing-endpoint' };
        const queue = getSyncQueue();
        if (!queue.length) return { ok: true, sent: 0 };
        const cursor = parseInt(localStorage.getItem(_key(K.SYNC_CURSOR)) || '0', 10);
        const pending = queue.slice(cursor).filter(i => i.status !== 'synced');
        if (!pending.length) return { ok: true, sent: 0 };
        const resp = await fetch(endpoint, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                userId: UserSystem.getCurrentUserId(),
                generatedAt: new Date().toISOString(),
                operations: pending,
            }),
        });
        if (!resp.ok) throw new Error(`sync-${resp.status}`);
        const synced = queue.map(item => ({ ...item, status: 'synced' }));
        _set(K.SYNC_QUEUE, synced);
        localStorage.setItem(_key(K.SYNC_CURSOR), String(queue.length));
        const reg = _get(K.REGISTRO).map(e => ({ ...e, sync_status: 'synced' }));
        _set(K.REGISTRO, reg);
        return { ok: true, sent: pending.length };
    }
    return { setUser, resetUser, saveSet, getRegistro, clearRegistro, saveOverride, getOverride, hasTodayEntries, getSyncQueue, setSyncEndpoint, getSyncEndpoint, flushSyncQueue };
})();
...
const UserSystem = (() => {
    ...
    function injectGokingHistory(userId) {
        ...
        const history = [
            ...
            // 18/03/2026 - HIPERTROFIA INF (Yesterday)
...
const UI = (() => {
    ...
    function renderDashboard() {
        ...
        if (mrecoEl) {
            const proHint = traffic.color === 'yellow'
                ? `Semáforo amarillo: carga 3 días ${traffic.carga3d} (límite ${Math.round(traffic.limit)}).`
                : '';
        ...
        if (sem) {
            const color = traffic.color === 'yellow' ? 'var(--yellow)' : 'var(--green)';
            sem.style.background = color;
            sem.style.boxShadow = `0 0 0 2px ${color}33`;
            sem.title = `Carga 3 días: ${traffic.carga3d} / ${Math.round(traffic.limit)} (${traffic.pct}%)`;
        }
    ...
    function buildExerciseCard(exData) {
        ...
        // Prioritize reco from plan data
        const recoSets = (exData && typeof exData === 'object' && exData.series) ? exData.series : (ej.fatiga === 3 ? 4 : ej.fatiga === 2 ? 4 : 3);
        const recoReps = (exData && typeof exData === 'object' && exData.reps) ? exData.reps : (ej.fatiga === 3 ? '6-8' : ej.fatiga === 2 ? '10-12' : '15');
        const recoPause = exData.pause || (ej.fatiga === 3 ? '180s' : ej.fatiga === 2 ? '90s' : '60s');
      ...
      <div class="exercise-card-header" onclick="UI.openLogModal('${cod}')">
        ...
        <div class="exercise-card-info">
          ...
          <p class="ex-reco">${recoSets} series × ${recoReps} • Descanso: ${recoPause}</p>
        </div>
        <div class="ex-weight-pill">${weightLabel}</div>
      </div>
          <div class="ex-reco-row">
            <span class="ex-reco-pill">${recoSets}×${recoReps}</span>
            <span class="ex-reco-pill pause">⏱ ${recoPause}</span>
            <span class="ex-group">${ej.grupo}</span>
          </div>
        </div>
      ...
      ${hasRirBadge ? `<div class="suggestion-banner" style="margin:8px 12px 0;"><span class="suggestion-icon">🟢</span><span class="suggestion-text">Última serie principal con RIR ≥ ${RIR_PROGRESSION_THRESHOLD}: subir carga</span></div>` : ''}
    ...
    function openLogModal(cod) {
        ...
        const user = UserSystem.getCurrentUser();
        const onboarding = user?.onboarding;
        const currentSem = Planner.getTodaySession()?.semana || 1;
    ...
    function initLogForm() {
        ...
        document.getElementById('log-form').addEventListener('submit', e => {
            ...
            DB.saveSet({ cod, fecha: Planner.todayStr(), peso, series, reps, rpe, rir, dolor, notes, vol_total: tonelaje, tonelaje, dolor_alert });
    ...
    window.UI = {
        ...
        openDiaDobleModal, closeDiaDobleModal,
        stepRIR, stepEndMin,
...
const LoginUI = {
    ...
    renderUserList() {
        ...
        list.innerHTML = users.map(u => {
            ...
            const obj = Translator.getObjetivoById(u.objetivo || 'ALTO_RENDIMIENTO');
            return `
                <div class="user-card" onclick="LoginUI.selectUser('${u.id}')">
                    <div class="user-card-emoji">${u.emoji || '⚡'}</div>
                    <div class="user-card-info">
                        <div class="user-card-name">${u.name}</div>
                        <div class="user-card-meta">${obj.label + ' · ' + stats.sessions + ' ses.'}</div>
                    </div>
                    <button class="btn-delete-user" onclick="event.stopImmediatePropagation(); LoginUI.deleteUser('${u.id}')">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg>
                    </button>
        ...
        if (loginForm) {
            loginForm.addEventListener('submit', e => {
                ...
                if (user) {
                    ...
                    alert('Usuario o contraseña incorrectos. Registrate si eres nuevo.');
                }
            });
        }
        ...
        if (newUserForm) {
            newUserForm.addEventListener('submit', e => {
                ...
                try {
                    ...
                    // Error already shown by createUser
                }
            });
        }

        const btnShowReg = document.getElementById('btn-show-register');
        if (btnShowReg) btnShowReg.addEventListener('click', () => {
...
const App = {
    init() {
        try {
            ...
            window.addEventListener('online', () => {
                DB.flushSyncQueue().catch(() => { /* reintento en próximo online */ });
            });
            window.addEventListener('focus', () => {
                if (navigator.onLine) DB.flushSyncQueue().catch(() => { /* no-op */ });
            });
            if (navigator.serviceWorker) {
                navigator.serviceWorker.addEventListener('message', e => {
                    if (e.data?.type === 'SYNC_NOW') {
                        DB.flushSyncQueue().catch(() => { /* no-op */ });
                    }
                });
            }
    ...
    confirmEndSession() {
        ...
        if (navigator.onLine) {
            DB.flushSyncQueue().catch(() => { /* intento best effort */ });
        }
