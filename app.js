...
const UserSystem = (() => {
    ...
    function _generatePlan(objetivoId) {
        ...
        if (onboarding) {
            ...
            const lesion       = onboarding.historialLesiones || 'ninguna';
            ...
            // 5. Ejercicios a excluir por lesión
            const LESION_EXCLUIR = {
                hombro: [1, 13, 42, 23, 45, 94, 19, 47, 93],   // press y empujes
                rodilla: [6, 10, 43, 98, 100, 9, 49],           // sentadillas y extensiones
                lumbar:  [7, 12, 41, 7, 102],                   // pesos muertos
            };
            const excluidos = LESION_EXCLUIR[lesion] || [];
...
const LoginUI = {
    ...
    _stepsBase: [
        ...
        },
        {
            ...
            title: '¿Tenés alguna molestia, lesión o zona que cuidar?',
            emoji: '🩺',
            desc: 'Marcá si hay algo que querés evitar forzar. El plan lo respeta.',
            options: [
                { label: 'No, estoy bien', val: 'ninguna' },
                { label: 'Hombro / Manguito rotador', val: 'hombro' },
                { label: 'Rodilla / Isquiotibiales', val: 'rodilla' },
                { label: 'Columna / Lumbar', val: 'lumbar' },
            ]
        },
        {
            id: 'recuperacion',
    ...
    _showSuperposicionAlert(equipo, gym, disponibles, total) {
        ...
        // Back goes back to last real step
        if (backBtn) backBtn.onclick = () => {
            this.onboardingStep = this.onboardingSteps.length - 1;
            this.renderOnboardingStep();
        };
        // Next confirms selected resolution
    ...
    _resolverSuperposicion(choice, confirm = false) {
        ...
        // Reset button onclick back to normal
        const nextBtn = document.getElementById('btn-onboarding-next');
