...
const UserSystem = (() => {
    ...
    function initAdmin() {
        ...
        if (!existing) {
            ...
            const admin = {
                ...
                password: 'G2026',
                emoji: '👨‍💼',
                objetivo: 'DUAL_FUTBOL',
                role: 'admin',
                status: 'active',
                created: new Date().toISOString(),
                onboarding: {
                    frecuenciaUlt7d: '3', disponibilidadDias: '5',
                    nivel: 'avanzado', duracionSesion: '75',
                    recuperacion: 'media', diaPartido: '0', diaDescanso: '6',
                    diasEquipo: '2', diasGym: '2', objetivoGym: 'fuerza',
                    diasEquipoEspecificos: '2,4',
                    historialLesiones: 'ninguna', equipamiento: 'completo',
                    objetivo_composicion: 'rendimiento', temporada: 'comp',
                    experienciaDeporte: 'experto',
                    edad: '28', peso: '78', altura: '178'
                }
            };
            cleaned.push(admin);
            saveUsers(cleaned);
    ...
    function injectGokingHistory(userId) {
        ...
        const injectedKey = `rabix_${userId}_history_injected`;
        // Never inject if already done once, or if there's real data (Supabase pull)
        if (localStorage.getItem(injectedKey)) return;
        if (localStorage.getItem(key)) {
            // Data exists — mark as injected to avoid future overwrites
    ...
    function _generatePlan(objetivoId) {
        ...
        // Usar generador dual si corresponde
        let plan = [];
        if (obj && obj.moduloDeportivo === 'dual' && window._generateDualPlan) {
            plan = window._generateDualPlan(objetivoId, onboarding);
        } else {
            plan = window._generatePlan ? window._generatePlan(objetivoId) : [];
        }
        if (!plan.length) return [];

        // ── Adaptación completa según onboarding ──
        if (onboarding) {
            const lvl          = onboarding.nivel || 'intermedio';
            const freq7d       = parseInt(onboarding.frecuenciaUlt7d || onboarding.frecuencia || '3', 10);
            const disponibles  = parseInt(onboarding.disponibilidadDias || '4', 10);
            const preferredMatchDow = parseInt(onboarding.diaPartido || '-1', 10);
            const preferredRestDow  = parseInt(onboarding.diaDescanso || '-1', 10);
            const lesion       = onboarding.historialLesiones || 'ninguna'; // may be comma-separated
            const objComp      = onboarding.objetivo_composicion || 'ganar';
            const equip        = onboarding.equipamiento || 'completo';
            const temporada    = onboarding.temporada || 'libre';
            const expDeporte   = onboarding.experienciaDeporte || 'medio';

            // 1. Volumen inicial según frecuencia reciente
            const volFactor = freq7d <= 1 ? 0.75 : freq7d <= 2 ? 0.85 : 1.0;

            // 2. Factor de nivel
            const nivelFactor = lvl === 'principiante' ? 0.80 : lvl === 'avanzado' ? 1.10 : 1.0;

            // 3. Factor de temporada (en competencia = menos volumen de sala)
            const tempFactor = temporada === 'comp' ? 0.80 : temporada === 'post' ? 0.70 : 1.0;

            // 3b. Factor de edad — recuperación más lenta con la edad
            const edad = parseInt(onboarding.edad || '25', 10);
            const ageFactor = edad < 25 ? 1.0 : edad < 35 ? 1.0 : edad < 45 ? 0.95 : 0.88;

            // 4. Readaptation factor — overrides everything if active
            const uid2 = UserSystem.getCurrentUserId();
            const readaptRaw = uid2 ? localStorage.getItem(`rabix_${uid2}_readapt`) : null;
            let readaptFactor = 1.0;
            let readaptRpeMax = null;
            if (readaptRaw) {
                try {
                    const rc = JSON.parse(readaptRaw);
                    const rcStart = new Date(rc.startDate + 'T12:00:00');
                    const rcEnd = new Date(rcStart); rcEnd.setDate(rcEnd.getDate() + rc.dias);
                    const now = new Date();
                    if (now <= rcEnd) {
                        readaptFactor = rc.volPct / 100;
                        readaptRpeMax = rc.rpeMax;
                    } else {
                        localStorage.removeItem(`rabix_${uid2}_readapt`); // expired
                    }
                } catch(e) {}
            }

            // 5. Factor combinado (no acumular más del 30% de reducción, readapt puede ir más bajo)
            const combinedFactor = readaptFactor < 1.0
                ? readaptFactor  // readaptation overrides other factors
                : Math.max(0.70, volFactor * nivelFactor * tempFactor * ageFactor);

            // 5. Ejercicios a excluir por lesión — multi-zone support
            const LESION_EXCLUIR = {
                hombro:   [1, 13, 42, 23, 45, 94, 19, 47, 93, 92],  // press, empujes, hombro
                codo:     [1, 13, 42, 46, 21, 20, 92],               // press cerrado, curl, extensiones
                cervical: [23, 45, 94, 96],                           // press vertical, remo al mentón
                lumbar:   [7, 12, 41, 102, 88],                       // pesos muertos y dragon flag
                cadera:   [6, 8, 43, 98, 99],                        // sentadilla profunda, hip thrust
                rodilla:  [6, 10, 43, 98, 100, 9, 49, 97],           // sentadillas y extensiones
                isquio:   [7, 41, 11, 101, 102],                     // RDL, isquio, nórdico
                tobillo:  [109, 110, 108, 107, 106],                  // sprints, saltos, agilidad
            };
            // Support comma-separated multi-zone: "hombro,rodilla"
            const zonas = (lesion || 'ninguna').split(',').filter(z => z !== 'ninguna');
            const excluidos = [...new Set(zonas.flatMap(z => LESION_EXCLUIR[z] || []))];

            // 6. Ejercicios a excluir por equipamiento
            const EQUIP_EXCLUIR = {
                pesas:   [100, 37, 48, 54, 111, 112, 113, 114], // máquinas y cardio específico
                minimo:  [100, 37, 48, 54, 111, 112, 113, 114, 1, 13, 42, 7, 12, 6],
                casa:    [100, 37, 48, 54, 111, 112, 113, 1, 13, 42, 7, 12, 6, 4, 30],
            };
            const excluidosEquip = EQUIP_EXCLUIR[equip] || [];
            const todosExcluidos = new Set([...excluidos, ...excluidosEquip]);

            plan = plan.map(s => {
                if (!Array.isArray(s.ejs_cods)) return s;

                // Ajustar series según factores combinados
                let ejs = s.ejs_cods.map(ex => {
                    if (typeof ex !== 'object') return ex;
                    const seriesBase = parseInt(ex.series || '3', 10);
                    const nSeries = Math.max(2, Math.round(seriesBase * combinedFactor));
                    return { ...ex, series: nSeries };
                });

                // Filtrar ejercicios por lesión y equipamiento
                if (todosExcluidos.size > 0) {
                    ejs = ejs.filter(ex => {
                        const cod = typeof ex === 'object' ? ex.cod : ex;
                        return !todosExcluidos.has(cod);
                    });
                    if (ejs.length === 0) ejs = s.ejs_cods; // fallback: no dejar vacío
                }

                // Respetar día de descanso preferido (gym puro)
                const dow = new Date(s.fecha + 'T12:00:00').getDay();
                let escenario = s.escenario;
                if (preferredRestDow >= 0 && dow === preferredRestDow && s.escenario !== 'PARTIDO') {
                    escenario = 'DESCANSO';
                    ejs = [];
                }

                // Respetar día de partido para deportes
                if (obj.moduloDeportivo === 'equipo') {
                    if (preferredMatchDow >= 0 && dow === preferredMatchDow) escenario = 'PARTIDO';
                    if (escenario !== s.escenario && escenario !== 'DESCANSO') {
                        ejs = [...(ESCENARIOS[escenario]?.ejercicios || [])].map(c => ({ cod: c, series: Math.round(3 * combinedFactor), reps: '8-12' }));
                    }
                }

                // Limitar sesiones por disponibilidad semanal
                // Si la semana ya tiene más sesiones activas que disponibles → convertir extras en DESCANSO
                // (se maneja a nivel de mapa de semana en cada semana)

                return { ...s, escenario, ejs_cods: ejs };
            });

            // 7. Limitar días activos por semana según disponibilidad
            if (disponibles < 5) {
                const semanas = {};
                plan.forEach(s => {
                    if (!semanas[s.semana]) semanas[s.semana] = [];
                    semanas[s.semana].push(s);
                });
                Object.values(semanas).forEach(dias => {
                    const activos = dias.filter(d => d.escenario !== 'DESCANSO' && d.escenario !== 'PARTIDO').sort((a,b) => {
                        // Mantener los primeros N días activos de la semana según disponibilidad
                        return new Date(a.fecha) - new Date(b.fecha);
                    });
                    if (activos.length > disponibles) {
                        // Convertir los excedentes en DESCANSO (los del final de la semana)
                        activos.slice(disponibles).forEach(s => {
                            s.escenario = 'DESCANSO';
                            s.ejs_cods = [];
                        });
                    }
                });
    ...
    function getPhysicalProfile(userId) {
        const uid = userId || getCurrentUserId();
        const user = getAllUsers().find(u => u.id === uid);
        const ob = user?.onboarding || {};
        const edad  = parseInt(ob.edad   || '25', 10);
        const peso  = parseFloat(ob.peso || '70');
        const altura = parseFloat(ob.altura || '170') / 100; // to meters
        const imc   = altura > 0 ? Math.round((peso / (altura * altura)) * 10) / 10 : null;
        const fcMax = Math.round(220 - edad); // Haskell-Fox formula
        const fcZ2  = { min: Math.round(fcMax * 0.60), max: Math.round(fcMax * 0.70) };
        const fcZ4  = { min: Math.round(fcMax * 0.85), max: Math.round(fcMax * 0.95) };
        // Mifflin-St Jeor TMB (assume male — no sex data yet, use neutral)
        const tmb   = Math.round(10 * peso + 6.25 * (altura * 100) - 5 * edad + 5);
        let imcLabel = '';
        if (imc !== null) {
        ...
        return { edad, peso, alturaM: altura, imc, imcLabel, fcMax, fcZ2, fcZ4, tmb };
    }
...
window.Planner = {
    ...
    todayStr() {
        // Use LOCAL date — avoids UTC offset bug (e.g. Uruguay UTC-3 shows next day after 21hs UTC)
        const d = new Date();
...
window.WeightEngine = {
    ...
    getInitialWeight(cod) {
        const base = PESOS_INICIALES[cod] ?? 0;
        if (base === 0) return 0; // bodyweight or cardio — no scaling
        const onboarding = UserSystem.getCurrentUser()?.onboarding || {};
        const userKg = parseFloat(onboarding.peso || '70');
        const refKg  = 70; // reference weight for PESOS_INICIALES values
        const nivel  = onboarding.nivel || 'intermedio';
        // Scale factor: 0.8 for beginner, 1.0 for intermediate, 1.1 for advanced
        const nivelScale = nivel === 'principiante' ? 0.8 : nivel === 'avanzado' ? 1.1 : 1.0;
        // Weight scale: linear but capped to avoid extreme values
        const weightScale = Math.max(0.5, Math.min(1.6, userKg / refKg));
        return Math.round(base * weightScale * nivelScale * 2.5) / 2.5; // round to nearest 2.5kg
    },
...
window.FatigueEngine = {
    ...
    getLast7Days() {
        ...
        const co = `${cutoff.getFullYear()}-${String(cutoff.getMonth()+1).padStart(2,'0')}-${String(cutoff.getDate()).padStart(2,'0')}`;
        return DB.getRegistro().filter(e => e.fecha >= co);
    },

    calcStats(entries) {
        if (!entries.length) return { rpe: null, vol: 0, sessions: 0 };
        // Session-level entries (partido, end-of-session RPE) count with full weight
        // Each session_rpe entry is expanded to match avg series count to avoid dilution
        const rpeVals = [];
        entries.filter(e => e.rpe > 0).forEach(e => {
            if (e.session_rpe) {
                // Weight partido/session entries as 4 entries (avg series per gym session)
                for (let i = 0; i < 4; i++) rpeVals.push(e.rpe);
            } else {
                rpeVals.push(e.rpe);
            }
        });
    ...
    getFatigueLimit() {
        ...
        // Recovery factor: sleep/stress quality
        const recFactor = rec === 'alta' ? 1.10 : rec === 'baja' ? 0.85 : 1.0;

        // Age factor: recovery slows with age
        // Under 25: +5%, 25-35: baseline, 35-45: -8%, 45+: -15%
        const ageFactor = edad < 25 ? 1.05 : edad < 35 ? 1.0 : edad < 45 ? 0.92 : 0.85;

        return obj.rpeYellow * sessionMin * 3 * recFactor * ageFactor;
    },
...
const AmistosoLogic = {
    applyFriendlyMatch(fecha) {
        ...
        DB.saveOverride(`${next.getFullYear()}-${String(next.getMonth()+1).padStart(2,'0')}-${String(next.getDate()).padStart(2,'0')}`, 'RECUPERACION');
    },
};
...
const UI = (() => {
    ...
    function switchView(viewId) {
        ...
        if (viewId === 'train' && DB.hasTodayEntries() && currentSession?.escenario !== 'DESCANSO') {
            // Session already done — redirect to history instead
            viewId = 'history';
            document.querySelectorAll('.view').forEach(v => v.classList.remove('active'));
            document.getElementById('view-history')?.classList.add('active');
            document.querySelectorAll('.nav-btn').forEach(b => b.classList.remove('active'));
            document.querySelector('.nav-btn[data-view="history"]')?.classList.add('active');
            renderHistory();
            return;
        }
    ...
    function renderDashboard() {
        ...
        if (preview) {
            const esc = currentSession.escenario || '';
            if (esc.startsWith('EQUIPO_')) {
                // Días de equipo: no mostrar ejercicios, solo mensaje informativo
                preview.innerHTML = '<span class="exercise-chip equipo-chip">🤝 Registrá minutos + RPE al terminar</span>';
            } else if (!currentSession.ejs_cods || currentSession.ejs_cods.length === 0) {
                preview.innerHTML = '';
            } else {
        ...
        // Readaptation protocol banner
        const uid4 = UserSystem.getCurrentUserId();
        const readaptActive = (() => {
            try {
                const rc = JSON.parse(localStorage.getItem(`rabix_${uid4}_readapt`) || 'null');
                if (!rc) return null;
                const end = new Date(rc.startDate + 'T12:00:00'); end.setDate(end.getDate() + rc.dias);
                return new Date() <= end ? rc : null;
            } catch { return null; }
        })();
        const readaptBannerEl = document.getElementById('readapt-banner');
        if (readaptBannerEl) {
            if (readaptActive) {
                const daysLeft = Math.ceil((new Date(readaptActive.startDate + 'T12:00:00').setDate(
                    new Date(readaptActive.startDate + 'T12:00:00').getDate() + readaptActive.dias
                ) - Date.now()) / 86400000);
                readaptBannerEl.classList.remove('hidden');
                const txt = readaptBannerEl.querySelector('.readapt-banner-text');
                if (txt) txt.textContent = `Protocolo de retorno activo — vol. ${readaptActive.volPct}%, RPE máx ${readaptActive.rpeMax}. Quedan aprox. ${Math.max(1, daysLeft)} día${daysLeft !== 1 ? 's' : ''}.`;
            } else {
                readaptBannerEl.classList.add('hidden');
            }
        }
        // Also consider sick days and readaptation as "session done"
        const todayEntries = DB.getRegistro().filter(e => e.fecha === Planner.todayStr());
        const isSickDay = todayEntries.some(e => e.escenario === 'DESCANSO_SALUD');
        const sessionDone = DB.hasTodayEntries();
        const startBtn = document.getElementById('btn-start-training');
        if (startBtn) {
            if (sessionDone && currentSession?.escenario !== 'DESCANSO') {
                startBtn.disabled = true;
                startBtn.classList.add('session-done');
                startBtn.innerHTML = `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg> SESIÓN REGISTRADA`;
            } else {
                startBtn.disabled = false;
                startBtn.classList.remove('session-done');
                startBtn.innerHTML = `<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><polygon points="5 3 19 12 5 21 5 3"/></svg> INICIAR SESIÓN`;
            }
        }
        ...
        if (diaDobleBtn) {
            if (sessionDone) {
                ...
                diaDobleBtn.classList.add('hidden');
            }
        }
        ...
        if (mvolEl) {
            if (fatigue.vol > 0) {
                mvolEl.textContent = `${(fatigue.vol / 1000).toFixed(1)}t`;
            } else {
                // Fallback: show carga interna acumulada when no tonelaje (eg. partido days)
                const cargaTotal = DB.getRegistro()
                    .filter(e => { const c = new Date(); c.setDate(c.getDate()-7); return e.fecha >= `${c.getFullYear()}-${String(c.getMonth()+1).padStart(2,'0')}-${String(c.getDate()).padStart(2,'0')}`; })
                    .reduce((s,e) => s + (e.carga_interna || 0), 0);
                mvolEl.textContent = cargaTotal > 0 ? `${Math.round(cargaTotal)} UI` : '—';
            }
        }
        ...
        const wbTopEl = document.getElementById('week-badge-top');
        if (wbTopEl) wbTopEl.textContent = `Sem. ${semana} / ${Planner.getTotalSemanas()}`;

        // Badge de módulo dual/híbrido
        const fatigueBadgeEl = document.getElementById('fatigue-badge');
        if (fatigueBadgeEl) {
            const mod = obj.moduloDeportivo || 'fuerza';
            if (mod === 'dual')   fatigueBadgeEl.textContent = 'Carga Dual 7d';
            else if (mod === 'hibrido') fatigueBadgeEl.textContent = 'Carga Híbrida 7d';
            else fatigueBadgeEl.textContent = '7 días';
        }
    ...
    function renderWeekDays(semana) {
        ...
        const containerTop = document.getElementById('week-days-top');
        ...
        const html = days.map(s => {
            ...
            const isSick = registro.some(e => e.fecha === s.fecha && e.escenario === 'DESCANSO_SALUD');
            const isReadapt = registro.some(e => e.fecha === s.fecha && e.escenario === 'READAPTACION');
            let cls = '';
            if (isToday) cls = 'today';
            else if (isSick) cls = 'sick';
            else if (isReadapt) cls = 'readapt';
            else if (isDone) cls = 'done';
            else if (isPast && !isRest) cls = 'skipped';
            else if (isRest) cls = 'rest';
            const icon = isSick ? '🤒' : isReadapt ? '🌱' : isDone ? '✓' : isToday ? '•' : (SCENARIO_EMOJI[s.escenario] || '—');
            return `<div class="day-dot ${cls}"><span>${DAY_LABELS[dow]}</span><div class="dot">${icon}</div></div>`;
        }).join('');
    ...
    function buildExerciseCard(exData) {
        ...
        // ── Compact set chips for logged sets ──
        const isCardio = ej.patron === 'Cardio' || ej.patron === 'HIIT' || ej.grupo === 'Aeróbico' || ej.grupo === 'Global';

        const setChipsHtml = hasLogs ? `
            <div class="ex-sets-chips">
                ${sets.map((s, i) => `
                <button class="ex-set-chip rpe-chip-${s.rpe <= 7 ? 'low' : s.rpe <= 8.5 ? 'mid' : 'high'}"
                    onclick="UI.editSet(${cod},${i})">
                    <span class="chip-num">S${i+1}</span>
                    <span class="chip-detail">${s.peso > 0 ? s.peso + 'kg' : 'PC'} · ${s.reps}rep</span>
                    <span class="chip-rpe">RPE ${s.rpe}</span>
                </button>`).join('')}
                <button class="ex-set-chip chip-add" onclick="UI.openLogModal(${cod})">+ Serie</button>
            </div>` : '';
        ...
        <div class="ex-card-header">
            <div class="ex-card-title-row">
                <div class="ex-card-info">
                    ...
                    <p class="ex-card-meta">${ej.grupo}${ej.patron ? ' · ' + ej.patron : ''}</p>
                </div>
            ...
            <div class="ex-params-row">
                <div class="ex-param-pill">
                    <span class="pill-val">${recoSets}</span>
                    <span class="pill-lbl">${isCardio ? 'veces' : 'series'}</span>
                </div>
                <span class="pill-dot">·</span>
                <div class="ex-param-pill">
                    <span class="pill-val">${recoReps}</span>
                    <span class="pill-lbl">${isCardio ? 'min' : 'reps'}</span>
                </div>
                ${!isCardio ? `
                <span class="pill-dot">·</span>
                <div class="ex-param-pill">
                    <span class="pill-val">${recoPause}</span>
                    <span class="pill-lbl">pausa</span>
                </div>` : ''}
                <div class="ex-param-pill pill-weight${isProgressed ? ' pill-prog' : ''}">
                    <span class="pill-val">${weightStr}</span>
                    <span class="pill-lbl">sugerido</span>
                </div>
            </div>
            ${progressionHint}
            ${painHint}
        ...
        ${!hasLogs && !isDone ? `
        <button class="ex-btn-log-main" onclick="UI.openLogModal(${cod})">
        ...
        <div class="ex-card-footer-actions">
            <button class="ex-btn-pain-sm${dolorEj >= 4 ? ' has-pain' : ''}"
                    onclick="App.openPainReport(${cod})">
                🩺 ${dolorEj >= 2 ? 'Dolor ' + dolorEj + '/10' : 'Sin molestias'}
            </button>
        </div>`
        return card;
    }
    ...
    function openLogModal(cod) {
        ...
        if (onboarding) {
            ...
            if (edad >= 40 && onboarding.nivel !== 'avanzado') defRpe = Math.min(defRpe, 6.5);
            if (edad >= 50) defSeries = Math.min(defSeries, 3);
        }
        // Readaptation protocol caps RPE and series
        const uid3 = UserSystem.getCurrentUserId();
        const readaptData = uid3 ? (() => { try { return JSON.parse(localStorage.getItem(`rabix_${uid3}_readapt`)); } catch { return null; } })() : null;
        if (readaptData) {
            const rcEnd = new Date(readaptData.startDate + 'T12:00:00'); rcEnd.setDate(rcEnd.getDate() + readaptData.dias);
            if (new Date() <= rcEnd) {
                defRpe = Math.min(defRpe, readaptData.rpeMax);
                defSeries = Math.min(defSeries, Math.max(2, Math.round(defSeries * readaptData.volPct / 100)));
            }
        }

        const isCardio = ej.patron === 'Cardio' || ej.patron === 'HIIT' || ej.grupo === 'Aeróbico' || ej.grupo === 'Global';

        // Cardio/aeróbico: series=1, reps=minutos (20-60), sin RIR, sin pausa
        if (isCardio) {
            document.getElementById('log-series').value = 1;
            document.getElementById('log-reps').value = 30;  // 30 min por defecto
            // Hide RIR for cardio — not relevant
            document.getElementById('rir-stepper')?.closest('.form-group')?.classList.add('hidden');
            document.getElementById('rir-badge')?.classList.add('hidden');
            // Change REPS label to MINUTOS
            const repsLabel = document.getElementById('log-reps')?.closest('.form-group')?.querySelector('.form-label');
            if (repsLabel) repsLabel.textContent = 'DURACIÓN (min)';
            // Hide SERIES label context
            const seriesLabel = document.getElementById('log-series')?.closest('.form-group')?.querySelector('.form-label');
            if (seriesLabel) seriesLabel.textContent = 'VECES';
        } else {
            ...
            // Restore labels in case previously changed
            const repsLabel = document.getElementById('log-reps')?.closest('.form-group')?.querySelector('.form-label');
            if (repsLabel) repsLabel.textContent = 'REPS';
            const seriesLabel = document.getElementById('log-series')?.closest('.form-group')?.querySelector('.form-label');
            if (seriesLabel) seriesLabel.textContent = 'SERIES';
            document.getElementById('rir-stepper')?.closest('.form-group')?.classList.remove('hidden');
        }
    ...
    function toggleDone(cod) {
        ...
        // Evitar doble tap accidental — solo guarda si no hay sets ya guardados esta sesión
        const alreadySaved = (loggedSets[cod] || []).length > 0;
        if (alreadySaved) {
            // Ya tiene series registradas — solo marcar como done visualmente, no duplicar
            loggedDone[cod] = true;
            refreshExerciseCard(cod);
            return;
        }
    ...
    function renderSessionLog() {
        ...
        const registro = DB.getRegistro();
        if (!registro.length) {
            log.innerHTML = `<div class="empty-state"><div class="empty-state-icon">📋</div><p>Sin sesiones registradas todavía.</p></div>`;
            return;
        }
        const byDate = {};
        ...
        log.innerHTML = sorted.slice(0, 30).map((fecha, idx) => {
            const entries = byDate[fecha];
            const session = Translator.getSessionByDate(fecha, Planner.getPlan());
            const escData = session ? (ESCENARIOS[session.escenario] || {}) : {};
            const label   = escData.label || session?.escenario || 'Sesión libre';
            const emoji   = escData.emoji || '🏋️';

            // Stats
            const exerciseEntries = entries.filter(e => e.cod > 0 && !e.session_rpe);
            const sessionRpe = entries.find(e => e.session_rpe);
            const vol = exerciseEntries.reduce((s, e) => s + (e.vol_total || 0), 0);
            const rpeVals = entries.filter(e => e.rpe > 0);
            const avgRpe = rpeVals.length ? (rpeVals.reduce((s,e) => s + e.rpe, 0) / rpeVals.length) : 0;
            const rpeCls = avgRpe > 8.5 ? 'high' : avgRpe > 7 ? 'mid' : 'low';

            // Group exercise entries by cod
            const byCod = {};
            exerciseEntries.forEach(e => {
                if (!byCod[e.cod]) byCod[e.cod] = [];
                byCod[e.cod].push(e);
            });

            // Build exercise rows for expanded view
            const exRows = Object.entries(byCod).map(([cod, sets]) => {
                const ej = Translator.getByCode(parseInt(cod));
                const name = ej ? ej.nombre : `Ejercicio ${cod}`;
                const isCardio = ej && (ej.patron === 'Cardio' || ej.patron === 'HIIT' || ej.grupo === 'Aeróbico' || ej.grupo === 'Global');
                const setsText = sets.map((s, i) => {
                    if (isCardio) return `${s.reps} min`;
                    const peso = s.peso > 0 ? `${s.peso} kg` : 'PC';
                    return `S${i+1}: ${s.reps}×${peso}`;
                }).join(' · ');
                const maxRpe = Math.max(...sets.map(s => s.rpe || 0));
                const rpeBadge = maxRpe > 0 ? `<span class="hist-rpe-badge ${maxRpe > 8.5 ? 'high' : maxRpe > 7 ? 'mid' : 'low'}">RPE ${maxRpe}</span>` : '';
                return `<div class="hist-ex-row">
                    <div class="hist-ex-name">${name}</div>
                    <div class="hist-ex-sets">${setsText} ${rpeBadge}</div>
                </div>`;
            }).join('');

            const hasData = exerciseEntries.length > 0;
            const domId = `sess-detail-${fecha}`;

            return `
            <div class="hist-session-card" onclick="UI.toggleSessionDetail('${fecha}')">
                <div class="hist-session-header">
                    <div class="hist-session-left">
                        <span class="hist-session-emoji">${emoji}</span>
                        <div>
                            <p class="hist-session-label">${label}</p>
                            <p class="hist-session-date">${_fmtShort(fecha)}</p>
                        </div>
                    </div>
                    <div class="hist-session-right">
                        ${vol > 0 ? `<span class="hist-stat">${(vol/1000).toFixed(1)}t</span>` : ''}
                        ${avgRpe > 0 ? `<span class="hist-rpe ${rpeCls}">${avgRpe.toFixed(1)}</span>` : ''}
                        <span class="hist-chevron" id="chev-${fecha}">›</span>
                    </div>
                </div>
                <div class="hist-session-body hidden" id="${domId}">
                    ${hasData ? exRows : '<p class="hist-empty">Solo carga registrada — sin ejercicios individuales.</p>'}
                    ${sessionRpe ? `<div class="hist-session-rpe-total">RPE sesión: <strong>${sessionRpe.rpe}</strong> · ${sessionRpe.minutos || '—'} min</div>` : ''}
                </div>
            </div>`;
        }).join('');
    }

    function toggleSessionDetail(fecha) {
        const body = document.getElementById(`sess-detail-${fecha}`);
        const chev = document.getElementById(`chev-${fecha}`);
        if (!body) return;
    ...
    function openObjetivoModal() {
        // Sync picker state with current objetivo
        const current = UserSystem.getCurrentObjetivo();
        const curObj  = OBJETIVOS_DISPONIBLES.find(o => o.id === current);
        const curMod  = curObj?.moduloDeportivo || 'fuerza';
        const curCat  = curObj?.categoria || 'GIMNASIO';

        // Pre-select current state in the multi-select picker
        if (curMod === 'dual') {
            // Find the deporte matching this dual objetivo
            const dualSub = CATEGORIAS_ENTRENAMIENTO.find(c => c.id === 'DUAL')
                ?.subcategorias.find(s => s.niveles?.[0]?.id === current);
            LoginUI._selectedDeporte = dualSub?.niveles?.[0]?.id || null;
            LoginUI._selectedGym     = null;
        } else if (curCat === 'GIMNASIO') {
            LoginUI._selectedDeporte = null;
            LoginUI._selectedGym     = current;
        } else if (curCat === 'DEPORTES' || curCat === 'HIBRIDO') {
            LoginUI._selectedDeporte = current;
            LoginUI._selectedGym     = null;
        } else {
            LoginUI._selectedDeporte = null;
            LoginUI._selectedGym     = current;
        }
        LoginUI.selectedObjetivo = current;

        // Render the shared multi-select picker into the modal container
        const container = document.getElementById('objetivo-options');
        container.innerHTML = '';
        LoginUI._renderMultiPicker(container);

        // Replace the confirm button behavior
        const confirmBtn = document.getElementById('btn-confirm-objetivo');
        if (confirmBtn) {
            confirmBtn.onclick = () => {
                const chosen = LoginUI.selectedObjetivo || current;
                UI.selectObjetivo(chosen);
            };
        }
    ...
    function init(skipRender = false) {
        if (!_initialized) {
            _initialized = true;
            initNav();
            initLogForm();
    ...
    function initGlobalListeners() {
        document.getElementById('btn-start-training').addEventListener('click', () => {
            const sess = currentSession || Planner.getTodaySession();
            const esc = sess.escenario || '';
            if (esc === 'PARTIDO' || esc === 'AMISTOSO') {
                App.openPartidoModal(sess);
            } else if (esc === 'DESCANSO') {
                App.openDescansoModal();
            } else if (esc.startsWith('EQUIPO_')) {
                // Días de equipo: solo registrar carga percibida, sin lista de ejercicios
                App.openEquipoModal(sess);
            } else {
                startTraining();
            }
        });
    ...
    function openLogExtraExercise() {
        // Abrir búsqueda de ejercicio extra en lugar de prompt del sistema
        UI.openExtraExerciseSearch();
    }
    ...
    function openExtraExerciseSearch() {
        const overlay = document.getElementById('extra-exercise-overlay');
        if (!overlay) return;
        // Populate search list
        const searchEl = document.getElementById('extra-ex-search');
        if (searchEl) searchEl.value = '';
        renderExtraExList('');
        overlay.classList.remove('hidden');
    }

    function renderExtraExList(query) {
        const list = document.getElementById('extra-ex-list');
        if (!list) return;
        const all = Translator.getAllExercises ? Translator.getAllExercises() : (typeof EJERCICIOS !== 'undefined' ? EJERCICIOS : []);
        const filtered = query.length >= 1
            ? all.filter(e => e.nombre.toLowerCase().includes(query.toLowerCase()) || e.grupo.toLowerCase().includes(query.toLowerCase()))
            : all.slice(0, 30);
        list.innerHTML = filtered.map(e => `
            <button type="button" class="extra-ex-row" onclick="UI.selectExtraExercise(${e.cod})">
                <span class="extra-ex-name">${e.nombre}</span>
                <span class="extra-ex-group">${e.grupo}</span>
            </button>`).join('') || '<p style="color:var(--text-3);padding:12px;text-align:center;">Sin resultados</p>';
    }

    function selectExtraExercise(cod) {
        document.getElementById('extra-exercise-overlay')?.classList.add('hidden');
        openLogModal(cod);
    }
...
const LoginUI = {
        ...
        if (newUserForm) {
            newUserForm.addEventListener('submit', e => {
                ...
                try {
                    ...
                    // Ir directo al cuestionario — el plan depende de esas respuestas
    ...
    onboardingStep: 0,
    onboardingData: {},
    // Base steps — todos los objetivos
    _stepsBase: [
        {
            id: 'frecuenciaUlt7d',
            title: '¿Cuántas veces entrenaste en la última semana?',
            emoji: '📅',
            desc: 'Nos ayuda a arrancar con una carga adecuada a tu estado actual.',
            options: [
                { label: '0 — vengo de un parón', val: '0' },
        ...
        },
        {
            ...
            emoji: '🗓️',
            desc: 'Contá solo los días que podés cumplir semana tras semana.',
            options: [
                { label: '2 días', val: '2' },
                { label: '3 días', val: '3' },
                { label: '4 días', val: '4' },
                { label: '5 días', val: '5' },
                { label: '6 días', val: '6' },
                { label: 'Todos los días', val: '7' },
            ]
        },
        {
            id: 'diaDescanso',
            title: '¿Qué día de la semana es tu descanso fijo?',
            emoji: '🛌',
            desc: 'El plan bloquea ese día. Si no tenés uno fijo elegí Flexible.',
            options: [
                { label: 'Lunes',     val: '1' },
                { label: 'Martes',    val: '2' },
        ...
        {
            id: 'nivel',
            title: '¿Cuánta experiencia tenés en el gimnasio?',
            emoji: '💪',
            desc: 'Ajustamos la carga inicial para que arranques bien, sin pasarte.',
            options: [
                { label: 'Principiante — menos de 6 meses', val: 'principiante' },
                { label: 'Intermedio — 1 a 2 años', val: 'intermedio' },
                { label: 'Avanzado — más de 3 años', val: 'avanzado' },
            ]
        },
        {
            id: 'duracionSesion',
            title: '¿Cuánto tiempo tenés por sesión de gimnasio?',
            emoji: '⏱️',
            desc: 'Ajusta la cantidad de ejercicios por día.',
            options: [
                { label: '30-45 min', val: '45' },
                { label: '60 min', val: '60' },
                { label: '75 min', val: '75' },
                { label: '90 min o más', val: '90' },
            ]
        },
        {
            id: 'equipamiento',
            title: '¿Dónde entrenás y qué equipamiento tenés?',
            emoji: '🏋️',
            desc: 'Así solo te mostramos ejercicios que podés hacer con lo que tenés.',
            options: [
                { label: 'Gimnasio completo — máquinas + pesas libres', val: 'completo' },
                { label: 'Pesas libres solamente', val: 'pesas' },
                { label: 'Cuerpo libre + algunas mancuernas', val: 'minimo' },
                { label: 'Solo peso corporal / casa', val: 'casa' },
            ]
        },
        {
            id: 'objetivo_composicion',
            title: '¿Qué querés lograr con el entrenamiento?',
            emoji: '🎯',
            desc: 'Define el tipo de trabajo que priorizamos en tu plan.',
            options: [
                { label: '💪 Ganar fuerza y músculo', val: 'ganar' },
                { label: '🔥 Perder grasa, mantener músculo', val: 'perder' },
                { label: '⚡ Rendimiento — velocidad y potencia', val: 'rendimiento' },
                { label: '🧘 Salud general y movilidad', val: 'salud' },
            ]
        },
        {
            id: 'historialLesiones',
            type: 'multiselect',
            title: '¿Tenés alguna zona que cuidar?',
            emoji: '🩺',
            desc: 'Podés marcar varias. El plan evita cargar esas zonas en exceso.',
            noneOption: { label: '✅ No tengo molestias', val: 'ninguna' },
            options: [
                { label: 'Hombro', val: 'hombro', icon: '💪' },
                { label: 'Codo / Muñeca', val: 'codo', icon: '🦾' },
                { label: 'Cervical / Cuello', val: 'cervical', icon: '🧠' },
                { label: 'Columna / Lumbar', val: 'lumbar', icon: '🦴' },
                { label: 'Cadera / Glúteo', val: 'cadera', icon: '🍑' },
                { label: 'Rodilla', val: 'rodilla', icon: '🦵' },
                { label: 'Isquiotibiales / Femoral', val: 'isquio', icon: '🏃' },
                { label: 'Tobillo / Pie', val: 'tobillo', icon: '🦶' },
            ]
        },
        {
            id: 'recuperacion',
            title: '¿Cómo estás descansando y manejando el estrés?',
            emoji: '😴',
            desc: 'Dormir bien y tener poco estrés acelera mucho la recuperación.',
            options: [
                { label: '✅ Bien — duermo 7-8h, estrés bajo', val: 'alta' },
                { label: '⚠️ Regular — sueño o estrés medio', val: 'media' },
                { label: '🔴 Mal — poco sueño o mucho estrés', val: 'baja' },
            ]
        },
        {
            id: 'edad',
            ...
            emoji: '🎂',
            desc: 'La edad ajusta los umbrales de fatiga y la velocidad de recuperación.',
            placeholder: 'Ej: 28',
            min: 14, max: 80, unit: 'años'
        },
        {
            id: 'peso',
            type: 'number',
            title: '¿Cuánto pesás?',
            emoji: '⚖️',
            desc: 'Escala las cargas iniciales de cada ejercicio a tu cuerpo.',
            placeholder: 'Ej: 75',
        ...
        },
        {
            ...
            title: '¿Cuánto medís?',
            emoji: '📏',
            desc: 'Junto con el peso calcula tu IMC y ajusta el plan si corresponde.',
            placeholder: 'Ej: 175',
    ...
    _stepsDeporte: [
        {
            id: 'diaPartido',
            title: '¿Qué día jugás o competís habitualmente?',
            emoji: '🏟️',
            desc: 'El día de partido, recuperación y activación se organizan alrededor de este.',
            options: [
                { label: 'Lunes',     val: '1' },
                { label: 'Martes',    val: '2' },
                { label: 'Miércoles', val: '3' },
                ...
                { label: 'Sábado',    val: '6' },
                { label: 'Domingo',   val: '0' },
                { label: 'Variable',  val: '-1' },
            ]
        },
        {
            id: 'experienciaDeporte',
            title: '¿Cuántos años llevás practicando tu deporte?',
            emoji: '🏅',
            desc: 'Define qué tanto peso le damos a la parte técnica vs. física.',
            options: [
                { label: 'Menos de 2 años', val: 'nuevo' },
                { label: '2 a 5 años', val: 'medio' },
                { label: 'Más de 5 años', val: 'experto' },
            ]
        },
        {
            id: 'temporada',
            title: '¿En qué momento de la temporada estás?',
            emoji: '📆',
            desc: 'Ajusta el tipo de carga según la etapa del año.',
            options: [
                { label: 'Pretemporada — empezando', val: 'pre' },
                { label: 'Temporada — compitiendo', val: 'comp' },
                { label: 'Post-temporada — descansando', val: 'post' },
                { label: 'No tengo temporada fija', val: 'libre' },
            ]
        },
    ],
    ...
    _stepsDual: [
        {
            ...
            title: '¿Qué día jugás o competís habitualmente?',
            emoji: '🏟️',
            desc: 'El plan distribuye recuperación, gym y equipo alrededor de este día.',
            options: [
                { label: 'Lunes',     val: '1' },
                { label: 'Martes',    val: '2' },
                { label: 'Miércoles', val: '3' },
        ...
        {
            id: 'diasEquipo',
            title: '¿Cuántos días entrenás con el equipo o en cancha?',
            emoji: '⚽',
            desc: 'Incluye prácticas, entrenamientos físicos y técnicos del equipo.',
            options: [
                { label: '1 día',  val: '1' },
                { label: '2 días', val: '2' },
                { label: '3 días', val: '3' },
        ...
        {
            id: 'diasGym',
            title: '¿Cuántos días por semana podés ir al gimnasio?',
            emoji: '🏋️',
            desc: 'Estos días son adicionales a los de equipo. Sé realista.',
            options: [
                { label: '1 día',  val: '1' },
                { label: '2 días', val: '2' },
                { label: '3 días', val: '3' },
        ...
        {
            id: 'objetivoGym',
            title: '¿Qué buscás lograr en el gimnasio?',
            emoji: '🎯',
            desc: 'Define qué tipo de sesiones priorizamos en sala.',
            options: [
                { label: '💪 Fuerza — más potencia y carga', val: 'fuerza' },
                { label: '⚡ Potencia explosiva — saltos y velocidad', val: 'potencia' },
                { label: '🛡️ Prevención — evitar lesiones', val: 'prevencion' },
            ]
        },
        {
            id: 'experienciaDeporte',
            title: '¿Cuántos años llevás en tu deporte?',
            emoji: '🏅',
            desc: 'Ajusta cuánta carga ponemos en sala vs. cancha.',
            options: [
                { label: 'Menos de 2 años', val: 'nuevo' },
                { label: '2 a 5 años', val: 'medio' },
                { label: 'Más de 5 años', val: 'experto' },
        ...
        },
        {
            ...
            title: '¿En qué momento de la temporada estás?',
            emoji: '📆',
            desc: 'En competencia reducimos la carga de sala para que llegués fresco.',
            options: [
                { label: 'Pretemporada', val: 'pre' },
                { label: 'Temporada — estoy compitiendo', val: 'comp' },
                { label: 'Post-temporada', val: 'post' },
                { label: 'Sin temporada fija', val: 'libre' },
            ]
        },
    ],
    ...
    _stepsHibrido: [
        {
            id: 'diasSala',
            title: '¿Cuántos días hacés pesas o sala?',
            emoji: '🏋️',
            desc: 'Sesiones de fuerza, acondicionamiento o trabajo físico en sala.',
            options: [
                { label: '2 días', val: '2' },
                { label: '3 días', val: '3' },
                { label: '4 días', val: '4' },
        ...
        {
            id: 'diasTecnica',
            title: '¿Cuántos días hacés técnica / sparring?',
            emoji: '🥊',
            desc: 'Los días de técnica cuentan como carga — el plan los respeta.',
            options: [
                { label: '1 día', val: '1' },
                { label: '2 días', val: '2' },
                { label: '3 días', val: '3' },
                { label: '4 o más días', val: '4' },
            ]
        },
        {
            id: 'temporada',
            title: '¿Estás en período de competencia activa?',
            emoji: '📆',
            desc: 'En competencia priorizamos técnica y reducimos volumen de sala.',
            options: [
                { label: 'Sí, compitiendo activamente', val: 'comp' },
                { label: 'No, en preparación', val: 'pre' },
                { label: 'Post-pelea / recuperación', val: 'post' },
            ]
        },
    ],

        // Computed: returns correct steps array based on current user objetivo
    get onboardingSteps() {
        const uid = UserSystem?.getCurrentUserId?.();
        const users = UserSystem?.getAllUsers?.() || [];
        const user = users.find(u => u.id === uid);
        const obj = user?.objetivo || this.currentOnboardingUser
            ? (UserSystem?.getAllUsers?.().find(u => u.id === this.currentOnboardingUser)?.objetivo || 'ALTO_RENDIMIENTO')
            : 'ALTO_RENDIMIENTO';

        const objData = typeof OBJETIVOS_DISPONIBLES !== 'undefined'
            ? OBJETIVOS_DISPONIBLES.find(o => o.id === obj)
            : null;
        const modulo = objData?.moduloDeportivo || 'fuerza';
        const cat    = objData?.categoria || 'GIMNASIO';

        if (modulo === 'dual') {
            return [...this._stepsBase, ...this._stepsDual];
        }
        if (modulo === 'hibrido') {
            return [...this._stepsBase, ...this._stepsHibrido];
        }
        if (cat === 'DEPORTES' || ['equipo','resistencia'].includes(modulo)) {
            return [...this._stepsBase, ...this._stepsDeporte];
        }
        return this._stepsBase;
    },
    ...
    showOnboarding(userId) {
        ...
        // Hide all views and modals cleanly
        ...
        document.querySelector('.bottom-nav')?.classList.add('hidden');
        document.querySelector('.app-main')?.classList.add('hidden');
        // Also hide login view explicitly
        document.getElementById('view-login')?.classList.add('hidden');
        document.getElementById('view-login')?.classList.remove('active');

        // Show onboarding — remove hidden BEFORE adding active
        const ov = document.getElementById('view-onboarding');
        if (ov) {
            ov.classList.remove('hidden');  // must remove hidden first
    ...
    renderOnboardingStep() {
        ...
        const current = this.onboardingStep + 1;
        const pct = Math.round((current / total) * 100);
        const container = document.getElementById('onboarding-step-container');
        if (!container) return;

        // Update progress bar and counter in header
        const progressBar = document.getElementById('onboarding-progress-bar');
        const progressPct = document.getElementById('onboarding-progress-pct');
        const progressStep = document.getElementById('onboarding-progress-step');
        if (progressBar) progressBar.style.width = pct + '%';
        if (progressPct) progressPct.textContent = pct + '%';
        if (progressStep) progressStep.textContent = current + ' de ' + total;

        // Update next/back buttons
        const nextBtn = document.getElementById('btn-onboarding-next');
        if (nextBtn) nextBtn.textContent = this.onboardingStep === total - 1 ? 'FINALIZAR ✓' : 'SIGUIENTE →';
        ...
        const currentVal = this.onboardingData[step.id] || '';

        if (step.type === 'multiselect') {
            // Multi-select step — stored as comma-separated string, e.g. "hombro,rodilla"
            const selected = new Set((this.onboardingData[step.id] || '').split(',').filter(Boolean));
            const isNoneSelected = selected.has('ninguna') || selected.size === 0;

            container.innerHTML = `
                <div class="onboarding-step-card">
                    ${step.emoji ? `<div class="onboarding-step-emoji">${step.emoji}</div>` : ''}
                    <p class="onboarding-step-title">${step.title}</p>
                    ${step.desc ? `<p class="onboarding-step-desc">${step.desc}</p>` : ''}
                    <div class="onboarding-options" style="gap:8px;">
                        ${step.noneOption ? `
                        <button class="onboarding-opt-btn ob-none-btn ${isNoneSelected ? 'selected' : ''}"
                            onclick="LoginUI.toggleLesionOption('ninguna', true)">
                            ${isNoneSelected ? '<span class="onboarding-opt-check">✓</span>' : ''}
                            ${step.noneOption.label}
                        </button>
                        <div class="ob-lesion-divider">o marcá las zonas que querés cuidar:</div>
                        ` : ''}
                        <div class="ob-lesion-grid">
                            ${step.options.map(opt => {
                                const isSel = selected.has(opt.val);
                                return `<button class="ob-lesion-btn ${isSel ? 'selected' : ''}"
                                    onclick="LoginUI.toggleLesionOption('${opt.val}', false)">
                                    <span class="ob-lesion-icon">${opt.icon || ''}</span>
                                    <span class="ob-lesion-label">${opt.label}</span>
                                </button>`;
                            }).join('')}
                        </div>
                    </div>
            ...
            container.innerHTML = `
                <div class="onboarding-step-card">
                    ...
                    <p class="onboarding-step-title">${step.title}</p>
                    ${step.desc ? `<p class="onboarding-step-desc">${step.desc}</p>` : ''}
                    <div class="ob-number-input-wrap">
                        <input
                            type="number"
                            ...
                            max="${step.max || 999}"
                            inputmode="numeric"
                            oninput="LoginUI.onboardingData['${step.id}'] = this.value"
                        />
                        <span class="ob-number-unit">${step.unit || ''}</span>
                    </div>
                </div>
            `;
            setTimeout(() => document.getElementById('ob-number-field')?.focus(), 80);
        } else {
                ...
                <div class="onboarding-step-card">
                    ...
                    <div class="onboarding-options">
                        ${step.options.map(opt => {
                            ...
                            return `<button class="onboarding-opt-btn ${isSelected ? 'selected' : ''}"
                                    onclick="LoginUI.selectOnboardingOption('${step.id}', '${opt.val}')">
                                ${isSelected ? '<span class="onboarding-opt-check">✓</span>' : ''}
                                ${opt.label}
                            </button>`;
                        }).join('')}
                    </div>
    ...
    toggleLesionOption(val, isNone) {
        const current = new Set((this.onboardingData['historialLesiones'] || '').split(',').filter(Boolean));
        if (isNone) {
            // "No tengo molestias" — clears everything
            this.onboardingData['historialLesiones'] = 'ninguna';
        } else {
            current.delete('ninguna'); // Remove none-flag if a zone is selected
            if (current.has(val)) {
                current.delete(val); // toggle off
            } else {
                current.add(val);
            }
            this.onboardingData['historialLesiones'] = current.size > 0
                ? [...current].join(',')
                : 'ninguna';
        }
        this.renderOnboardingStep();
    ...
    nextOnboardingStep() {
        // Superposition screen intercept — must check before touching onboardingSteps
        if (this._inSuperposicion) {
            if (!this._superposicionChoice) {
                const card = document.querySelector('.superposicion-card');
                if (card) { card.style.border = '1.5px solid var(--orange)'; setTimeout(() => card.style.border = '', 900); }
                return;
            }
            this._inSuperposicion = false;
            this._resolverSuperposicion(this._superposicionChoice, true);
            return;
        }

        const step = this.onboardingSteps[this.onboardingStep];
        const val = this.onboardingData[step.id];
        const isEmpty = val === undefined || val === null || val === '';
        const isInvalidNumber = step.type === 'number' && (isEmpty || isNaN(Number(val)) || Number(val) < (step.min||0) || Number(val) > (step.max||999));
        // Multiselect always has a value (defaults to 'ninguna' on toggle)
        const isMultiselect = step.type === 'multiselect';

        if (!isMultiselect && (isEmpty || isInvalidNumber)) {
            const card = document.querySelector('.onboarding-step-card');
            if (card) { card.style.border = '1.5px solid var(--orange)'; setTimeout(() => card.style.border = '', 900); }
            if (step.type === 'multiselect') {
            // Multi-select step — stored as comma-separated string, e.g. "hombro,rodilla"
            const selected = new Set((this.onboardingData[step.id] || '').split(',').filter(Boolean));
            ...
            `;
                ...
                const field = document.getElementById('ob-number-field');
                if (field) field.focus();
            }
            return;
        }
        // Normalize number inputs to string
        if (step.type === 'multiselect') {
            // Multi-select step — stored as comma-separated string, e.g. "hombro,rodilla"
            const selected = new Set((this.onboardingData[step.id] || '').split(',').filter(Boolean));
            const isNoneSelected = selected.has('ninguna') || selected.size === 0;
            ...
            this.onboardingData[step.id] = String(Number(val));
        }
    ...
    finishOnboarding() {
        ...
        // ── Validación de superposición días equipo + gym ──
        const isDual = (() => {
            const uid = this.currentOnboardingUser || UserSystem.getCurrentUserId();
            const users = UserSystem.getAllUsers();
            const u = users.find(x => x.id === uid);
            const obj = typeof OBJETIVOS_DISPONIBLES !== 'undefined'
                ? OBJETIVOS_DISPONIBLES.find(o => o.id === u?.objetivo)
                : null;
            return obj?.moduloDeportivo === 'dual';
        })();

        if (isDual && data.diasEquipo && data.diasGym && data.disponibilidadDias) {
            const equipo      = parseInt(data.diasEquipo, 10);
            const gym         = parseInt(data.diasGym, 10);
            const disponibles = parseInt(data.disponibilidadDias, 10);
            const total       = equipo + gym;

            if (total > disponibles) {
                // Superposición detectada — mostrar alerta con opciones
                this._showSuperposicionAlert(equipo, gym, disponibles, total);
    ...
    _showSuperposicionAlert(equipo, gym, disponibles, total) {
        // Remove any existing alert
        document.getElementById('superposicion-alert')?.remove();

        const container = document.getElementById('onboarding-step-container');
        if (!container) { this._saveAndLaunch(); return; }

        const puedenCoincidir = total - disponibles; // días que se superponen

        container.innerHTML = `
            <div class="onboarding-step-card superposicion-card">
                <div class="onboarding-step-emoji">⚠️</div>
                <p class="onboarding-step-title">Hay una superposición de días</p>
                <p class="onboarding-step-desc">
                    Elegiste <strong>${equipo} días de equipo + ${gym} días de gym = ${total} días</strong>,
                    pero dijiste que podés entrenar <strong>${disponibles} días</strong> por semana.
                    Hay <strong>${puedenCoincidir} día${puedenCoincidir > 1 ? 's' : ''}</strong> que se superpondrían.
                </p>
                <p class="onboarding-step-desc" style="margin-top:8px;">¿Cómo querés manejarlo?</p>
                <div class="onboarding-options" style="margin-top:16px;">
                    <button class="onboarding-opt-btn" onclick="LoginUI._resolverSuperposicion('doble')">
                        <span>💪 Días dobles — gym + equipo el mismo día</span>
                        <span style="font-size:11px;color:var(--text-3);display:block;margin-top:4px;">Sesión de sala por la mañana, equipo por la tarde (o viceversa)</span>
                    </button>
                    <button class="onboarding-opt-btn" onclick="LoginUI._resolverSuperposicion('dividir')">
                        <span>📅 Dividir en ${total} días — aumentar disponibilidad</span>
                        <span style="font-size:11px;color:var(--text-3);display:block;margin-top:4px;">Actualiza tu disponibilidad a ${total} días y cada sesión es independiente</span>
                    </button>
                    <button class="onboarding-opt-btn" onclick="LoginUI._resolverSuperposicion('reducir')">
                        <span>✂️ Reducir gym a ${disponibles - equipo} día${disponibles - equipo !== 1 ? 's' : ''} — respetar disponibilidad</span>
                        <span style="font-size:11px;color:var(--text-3);display:block;margin-top:4px;">Priorizás el equipo y ajustás el gym a lo que te queda</span>
                    </button>
                </div>
            </div>`;
        ...
        const nextBtn = document.getElementById('btn-onboarding-next');
        if (nextBtn) nextBtn.textContent = 'CONFIRMAR';
        const backBtn = document.getElementById('btn-onboarding-back');
        if (backBtn) backBtn.classList.remove('hidden');
        // Back goes back to last real step — also resets superposition state
        if (backBtn) backBtn.onclick = () => {
            this._inSuperposicion = false;
            this._superposicionChoice = null;
            backBtn.onclick = null;
            this.onboardingStep = this.onboardingSteps.length - 1;
            this.renderOnboardingStep();
        };
        // Set flag — nextOnboardingStep checks this instead of using onclick
        this._inSuperposicion = true;
        this._superposicionChoice = null;
    },

    _resolverSuperposicion(choice, confirm = false) {
        this._superposicionChoice = choice;
        const data = this.onboardingData;
        const equipo      = parseInt(data.diasEquipo, 10);
        const gym         = parseInt(data.diasGym, 10);
        const disponibles = parseInt(data.disponibilidadDias, 10);
        ...
        // Highlight selection
        document.querySelectorAll('.superposicion-card .onboarding-opt-btn').forEach((btn, i) => {
            const map = ['doble','dividir','reducir'];
            btn.classList.toggle('selected', map[i] === choice);
        });

        if (!confirm) return; // Just highlight, wait for CONFIRMAR

        // Apply the chosen resolution
        if (choice === 'doble') {
            // Mark overlap days as double sessions in onboarding
            data.diasDobleEquipoGym = 'true';
            data.disponibilidadDias = String(disponibles); // keep original
        } else if (choice === 'dividir') {
            // Increase disponibilidad to fit all sessions
            data.disponibilidadDias = String(equipo + gym);
            data.diasDobleEquipoGym = 'false';
        } else if (choice === 'reducir') {
            // Reduce gym days to fit
            data.diasGym = String(Math.max(1, disponibles - equipo));
            data.diasDobleEquipoGym = 'false';
        }
    ...
    // Picker state — multi-select flat
    _selectedDeporte: null,  // objetivo de deporte seleccionado (o null)
    _selectedGym: null,      // objetivo de gym seleccionado (o null)
    _pickerLevel: 0,         // 0=main, 1=deporte niveles, 2=gym subs (legacy compat)
    _pickerCat: null,
    _pickerSub: null,
    ...
    renderObjetivoPicker() {
        ...
        if (!picker) return;
        picker.innerHTML = '';
        this._renderMultiPicker(picker);
    },

    _renderMultiPicker(picker) {
        // ── SECCIÓN DEPORTES ──
        const sportHeader = document.createElement('div');
        sportHeader.className = 'mpicker-section-header';
        sportHeader.innerHTML = '⚽ Deporte <span class="mpicker-hint">Opcional</span>';
        picker.appendChild(sportHeader);

        // Deportes en grilla 3 cols
        const sportGrid = document.createElement('div');
        sportGrid.className = 'mpicker-sport-grid';

        const deportesCat = CATEGORIAS_ENTRENAMIENTO.find(c => c.id === 'DEPORTES');
        if (deportesCat) {
            deportesCat.subcategorias.forEach(sub => {
                const hasLevels = sub.niveles && sub.niveles.length > 1;
                const singleId  = !hasLevels ? sub.niveles[0].id : null;
                const isSelected = this._selectedDeporte &&
                    (this._selectedDeporte === singleId ||
                     (hasLevels && sub.niveles.some(n => n.id === this._selectedDeporte)));

                const btn = document.createElement('button');
                btn.type = 'button';
                btn.className = 'mpicker-sport-btn' + (isSelected ? ' selected' : '');
                btn.innerHTML = `<span class="mpicker-emoji">${sub.emoji}</span><span class="mpicker-lbl">${sub.label}</span>`;

                if (hasLevels) {
                    // Show inline level selector if this sport is selected
                    btn.onclick = () => {
                        if (isSelected) {
                            this._selectedDeporte = null;
                        } else {
                            // Open level picker inline
                            this._pendingSportSub = sub;
                            this._selectedDeporte = sub.niveles[0].id; // default to first
                        }
                        this._updateSelectedObjetivo();
                        this.renderObjetivoPicker();
                    };
                } else {
                    btn.onclick = () => {
                        this._selectedDeporte = isSelected ? null : singleId;
                        this._updateSelectedObjetivo();
                        this.renderObjetivoPicker();
                    };
                }
                ...
                // If selected and has levels — show level pills inline below
                if (isSelected && hasLevels) {
                    const levelRow = document.createElement('div');
                    levelRow.className = 'mpicker-level-row';
                    sub.niveles.forEach(niv => {
                        const lbtn = document.createElement('button');
                        lbtn.type = 'button';
                        lbtn.className = 'mpicker-level-pill' + (this._selectedDeporte === niv.id ? ' selected' : '');
                        lbtn.textContent = niv.label;
                        lbtn.onclick = () => {
                            this._selectedDeporte = niv.id;
                            this._updateSelectedObjetivo();
                            this.renderObjetivoPicker();
                        };
                        levelRow.appendChild(lbtn);
                    });
                    // Insert after this button in the grid
                    sportGrid.appendChild(levelRow);
                }
            });
        }
        picker.appendChild(sportGrid);

        // ── SECCIÓN GIMNASIO ──
        const gymHeader = document.createElement('div');
        gymHeader.className = 'mpicker-section-header';
        gymHeader.style.marginTop = '20px';
        gymHeader.innerHTML = '🏋️ Objetivo en Sala <span class="mpicker-hint">Opcional</span>';
        picker.appendChild(gymHeader);

        const gymGrid = document.createElement('div');
        gymGrid.className = 'mpicker-gym-grid';

        const gymCat = CATEGORIAS_ENTRENAMIENTO.find(c => c.id === 'GIMNASIO');
        if (gymCat) {
            gymCat.subcategorias.forEach(sub => {
                const obj = OBJETIVOS_DISPONIBLES.find(o => o.selectorId === sub.id);
                const objId = obj ? obj.id : sub.id;
                const isSelected = this._selectedGym === objId;

                const btn = document.createElement('button');
                btn.type = 'button';
                btn.className = 'mpicker-gym-btn' + (isSelected ? ' selected' : '');
                btn.innerHTML = `<span class="mpicker-emoji">${sub.emoji}</span><span class="mpicker-lbl">${sub.label}</span>`;
                btn.onclick = () => {
                    this._selectedGym = isSelected ? null : objId;
                    this._updateSelectedObjetivo();
                    this.renderObjetivoPicker();
                };
                gymGrid.appendChild(btn);
            });
        }
        picker.appendChild(gymGrid);

        // ── SELECCIÓN ACTUAL ──
        const current = document.createElement('div');
        current.className = 'mpicker-current';
        const resolvedLabel = this._getResolvedLabel();
        current.innerHTML = resolvedLabel
            ? `<span class="mpicker-current-icon">✓</span> <span>${resolvedLabel}</span>`
            : '<span class="mpicker-current-empty">Seleccioná al menos una opción</span>';
        picker.appendChild(current);
    },

    _updateSelectedObjetivo() {
        const sport = this._selectedDeporte;
        const gym   = this._selectedGym;

        if (sport && gym) {
            // Try to find DUAL objective
            const dualId = this._getDualId(sport);
            this.selectedObjetivo = dualId || sport;
        } else if (sport) {
            this.selectedObjetivo = sport;
        } else if (gym) {
            this.selectedObjetivo = gym;
        } else {
            this.selectedObjetivo = 'ALTO_RENDIMIENTO'; // default
        }
    },

    _getDualId(deporteObjetivoId) {
        // Map from deporte objetivo to DUAL objective
        const dualMap = {
            FUTBOL_PRO: 'DUAL_FUTBOL', FUTBOL_SEMI: 'DUAL_FUTBOL', FUTBOL_AMATEUR: 'DUAL_FUTBOL',
            TENIS_GEN: 'DUAL_TENIS', HOCKEY_GEN: 'DUAL_HOCKEY',
            RUGBY_GEN: 'DUAL_RUGBY', HANDBALL_GEN: 'DUAL_HANDBALL',
            BALONCESTO_GEN: 'DUAL_BALONCESTO', ATLETISMO_GEN: 'DUAL_ATLETISMO',
            NATACION_GEN: 'DUAL_NATACION',
            VOLLEY_COMPETITIVO: 'DUAL_VOLLEY', VOLLEY_GEN: 'DUAL_VOLLEY',
        };
        return dualMap[deporteObjetivoId] || null;
    },
    ...
    _getResolvedLabel() {
        ...
        const getLabel = id => {
            const o = OBJETIVOS_DISPONIBLES.find(x => x.id === id);
            return o ? o.label : id;
        };

        if (sport && gym) {
            const dualId = this._getDualId(sport);
            if (dualId) return getLabel(dualId) + ' (Plan Dual)';
            return getLabel(sport) + ' + ' + getLabel(gym);
        }
        if (sport) return getLabel(sport);
        if (gym)   return getLabel(gym);
        return null;
    },

    _renderPickerCats(picker) {
        // Tarjeta destacada para DUAL
        const dualCat = CATEGORIAS_ENTRENAMIENTO.find(c => c.id === 'DUAL');
        if (dualCat) {
            const featured = document.createElement('button');
            featured.type = 'button';
            featured.className = 'picker-cat-featured';
            featured.innerHTML = `
                <div class="picker-cat-featured-left">
                    <span class="picker-cat-featured-emoji">${dualCat.emoji}</span>
                    <div>
                        <span class="picker-cat-featured-label">${dualCat.label}</span>
                        <span class="picker-cat-featured-desc">${dualCat.desc}</span>
                    </div>
                </div>
                <span class="picker-cat-featured-arrow">›</span>`;
            featured.onclick = () => {
                this._pickerCat = 'DUAL';
                this._pickerLevel = 1;
                this.renderObjetivoPicker();
            };
            picker.appendChild(featured);
        }

        // Grilla para el resto
        const wrap = document.createElement('div');
        wrap.className = 'picker-cat-grid';
        CATEGORIAS_ENTRENAMIENTO.filter(c => c.id !== 'DUAL').forEach(cat => {
            const btn = document.createElement('button');
            btn.type = 'button';
            btn.className = 'picker-cat-card';
            btn.innerHTML = `
                <span class="picker-cat-emoji">${cat.emoji}</span>
                <span class="picker-cat-label">${cat.label}</span>
                <span class="picker-cat-count">${cat.subcategorias.length} opciones</span>`;
            btn.onclick = () => {
                this._pickerCat = cat.id;
                this._pickerLevel = 1;
                this.renderObjetivoPicker();
            };
    ...
    _renderPickerSubs(picker) {
        const cat = CATEGORIAS_ENTRENAMIENTO.find(c => c.id === this._pickerCat);
        if (!cat) return;

        // Back button
        picker.appendChild(this._pickerBackBtn());

        // Category header
        const hdr = document.createElement('div');
        hdr.className = 'picker-sub-header';
        hdr.innerHTML = `<span>${cat.emoji}</span><span>${cat.label}</span>`;
        picker.appendChild(hdr);

        const grid = document.createElement('div');
        grid.className = 'picker-sub-grid';

        if (cat.id === 'GIMNASIO') {
            // Gimnasio: selección directa de objetivo
            cat.subcategorias.forEach(sub => {
                ...
                const btn = document.createElement('button');
                btn.type = 'button';
                btn.className = 'picker-sub-btn' + (isSelected ? ' selected' : '');
                btn.style.setProperty('--sub-color', obj?.color || '#659499');
                btn.innerHTML = `
                    <span class="picker-sub-emoji">${sub.emoji}</span>
                    <span class="picker-sub-label">${sub.label}</span>`;
                btn.onclick = () => this.pickObjetivo(objId);
                grid.appendChild(btn);
            });
        } else {
            // Deportes: mostrar deporte, luego despliega niveles si aplica
            cat.subcategorias.forEach(sub => {
                const hasManyLevels = sub.niveles && sub.niveles.length > 1;
                const singleObj = !hasManyLevels
                    ? OBJETIVOS_DISPONIBLES.find(o => o.id === sub.niveles[0].id)
                    : null;
                const isSelected = !hasManyLevels && this.selectedObjetivo === sub.niveles[0].id;

                const btn = document.createElement('button');
                btn.type = 'button';
                btn.className = 'picker-sub-btn' + (isSelected ? ' selected' : '');
                btn.style.setProperty('--sub-color', singleObj?.color || '#659499');
                btn.innerHTML = `
                    <span class="picker-sub-emoji">${sub.emoji}</span>
                    <span class="picker-sub-label">${sub.label}</span>
                    ${hasManyLevels ? '<span class="picker-sub-arrow">›</span>' : ''}`;

                if (hasManyLevels) {
                    btn.onclick = () => {
                        this._pickerSub = sub.id;
                        this._pickerLevel = 2;
                        this.renderObjetivoPicker();
                    };
                } else {
                    btn.onclick = () => this.pickObjetivo(sub.niveles[0].id);
                }
                grid.appendChild(btn);
            });
        }
    ...
    _renderPickerNiveles(picker) {
        const cat = CATEGORIAS_ENTRENAMIENTO.find(c => c.id === this._pickerCat);
        const sub = cat?.subcategorias.find(s => s.id === this._pickerSub);
        if (!sub) return;
        ...
        const hdr = document.createElement('div');
        hdr.className = 'picker-sub-header';
        hdr.innerHTML = `<span>${sub.emoji}</span><span>${sub.label} — elegí tu nivel</span>`;
        picker.appendChild(hdr);

        const list = document.createElement('div');
        list.className = 'picker-nivel-list';
        ...
        sub.niveles.forEach(niv => {
            ...
            const btn = document.createElement('button');
            btn.type = 'button';
            btn.className = 'picker-nivel-btn' + (isSelected ? ' selected' : '');
            btn.style.setProperty('--niv-color', obj?.color || '#659499');
            btn.innerHTML = `
                <div class="picker-nivel-info">
                    <span class="picker-nivel-label">${niv.label}</span>
                    <span class="picker-nivel-desc">${niv.desc || ''}</span>
                </div>
                ${isSelected ? '<span class="picker-nivel-check">✓</span>' : ''}`;
            btn.onclick = () => this.pickObjetivo(niv.id);
    ...
    _pickerBackBtn() {
        const btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'picker-back-btn';
        btn.innerHTML = '← Volver';
        btn.onclick = () => {
            if (this._pickerLevel === 2) { this._pickerLevel = 1; this._pickerSub = null; }
            else { this._pickerLevel = 0; this._pickerCat = null; }
            this.renderObjetivoPicker();
        };
        return btn;
    },
    ...
    pickObjetivo(id) {
        this.selectedObjetivo = id;
        this.renderObjetivoPicker();
    },

    _resetPicker() {
        this._pickerLevel = 0;
        this._pickerCat = null;
        this._pickerSub = null;
        this._selectedDeporte = null;
        this._selectedGym = null;
    },
};
...
const App = {
    init() {
        try {
            ...
            if (uid) {
                ...
                this.reinitWithObjetivo(user?.objetivo || 'ALTO_RENDIMIENTO', true);
                this.checkYesterday();
                // Render immediately with local data (page reload — localStorage already populated)
                // Then silently refresh after cloud sync for any updates
                UI.renderDashboard();
                DB.pullFromCloud(uid)
                    .then(() => { UI.renderDashboard(); })
                    .catch(() => {});
            } else {
                window.LoginUI.show();
            }
    ...
    initForUser(userId) {
        ...
        window.LoginUI.hide();
        // Show loading state while pulling cloud data
        const todayCard = document.getElementById('today-card');
        if (todayCard) {
            todayCard.style.opacity = '0.4';
            todayCard.style.pointerEvents = 'none';
        }
        const startBtn = document.getElementById('btn-start-training');
        if (startBtn) { startBtn.disabled = true; startBtn.innerHTML = '⏳ Cargando datos...'; }

        // Skip immediate render — wait for cloud data before showing dashboard
        this.reinitWithObjetivo(user.objetivo || 'ALTO_RENDIMIENTO', true);
        this.checkYesterday();
        // Pull cloud data first, then render with complete data
        DB.pullFromCloud(userId)
            .then(() => {
                if (todayCard) { todayCard.style.opacity = ''; todayCard.style.pointerEvents = ''; }
                UI.renderDashboard();
            })
            .catch(() => {
                if (todayCard) { todayCard.style.opacity = ''; todayCard.style.pointerEvents = ''; }
                UI.renderDashboard(); // Render with local data if cloud fails
            });
    },
    ...
    reinitWithObjetivo(objetivoId, skipRender = false) {
    ...
    checkYesterday() {
        ...
        // Count consecutive days without any training entry (looking back from yesterday)
        let diasSinEntrenar = 0;
        for (let i = 1; i <= 14; i++) {
            const d = new Date(); d.setDate(d.getDate() - i);
            const dStr = `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;
            // Stop counting if we find a training day or a recorded sick/rest day
            const hasActivity = registro.some(e => e.fecha === dStr);
            if (hasActivity) break;
            diasSinEntrenar++;
        }

        if (diasSinEntrenar === 0) return; // trained yesterday, all good

        if (diasSinEntrenar >= 2) {
            // Multi-day absence — show illness/absence protocol modal
            this._diasSinEntrenar = diasSinEntrenar;
            setTimeout(() => {
                document.getElementById('illness-modal-overlay')?.classList.remove('hidden');
                this._renderIllnessModal(diasSinEntrenar);
            }, 800);
        } else {
            // Just yesterday — simple modal
    ...
    _renderIllnessModal(dias) {
        const el = document.getElementById('illness-days-count');
        if (el) el.textContent = dias === 1 ? 'ayer' : `los últimos ${dias} días`;
    },

    dismissIllness() {
        document.getElementById('illness-modal-overlay')?.classList.add('hidden');
    },

    reportIllness(tipo) {
        // tipo: 'leve' | 'fuerte' | 'retorno'
        document.getElementById('illness-modal-overlay')?.classList.add('hidden');

        const hoy = this.todayStr();
        const dias = this._diasSinEntrenar || 1;

        if (tipo === 'fuerte') {
            // Register all missed days as DESCANSO_SALUD
            for (let i = dias; i >= 1; i--) {
                const d = new Date(); d.setDate(d.getDate() - i);
                const dStr = `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;
                DB.saveSet({
                    cod: 0, fecha: dStr, peso: 0, series: 1, reps: 0, rpe: 0,
                    vol_total: 0, carga_interna: 0, session_rpe: true,
                    escenario: 'DESCANSO_SALUD',
                    notes: '🤒 Descanso por enfermedad (síntomas fuertes)',
                });
            }
            // Also mark today as rest
            DB.saveSet({
                cod: 0, fecha: hoy, peso: 0, series: 1, reps: 0, rpe: 0,
                vol_total: 0, carga_interna: 0, session_rpe: true,
                escenario: 'DESCANSO_SALUD',
                notes: '🤒 Descanso por enfermedad — continuación',
            });
            // Inject recovery overrides for next 2 days
            for (let i = 1; i <= 2; i++) {
                const d = new Date(); d.setDate(d.getDate() + i);
                const dStr = `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;
                DB.saveOverride(dStr, 'RECUPERACION_FACILITADA');
            }
            UI.renderDashboard();
            setTimeout(() => alert('✅ Días registrados. Descansá bien — el plan te espera sin presión.'), 200);

        } else if (tipo === 'leve') {
            // Register missed days as sick rest
            for (let i = dias; i >= 1; i--) {
                const d = new Date(); d.setDate(d.getDate() - i);
                const dStr = `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;
                DB.saveSet({
                    ...
                    vol_total: 0, carga_interna: 0, session_rpe: true,
                    escenario: 'DESCANSO_SALUD',
                    notes: '🤧 Descanso por enfermedad (síntomas leves)',
                });
            }
            // Today: offer light mobility session
            const mobilitySession = {
                fecha: hoy,
                escenario: 'MOVILIDAD_SUAVE',
                ejs_cods: [55, 56, 59, 64, 60, 63].map(c => ({ cod: c, series: 1, reps: '60s' })),
                semana: Planner.getTodaySession()?.semana || 1,
            };
            UI.startTrainingForSession(mobilitySession);

        } else if (tipo === 'retorno') {
            // Register missed days as sick rest
            for (let i = dias; i >= 0; i--) {
                if (i === 0) continue; // today handled separately
                const d = new Date(); d.setDate(d.getDate() - i);
                const dStr = `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;
                DB.saveSet({
                    cod: 0, fecha: dStr, peso: 0, series: 1, reps: 0, rpe: 0,
            ...
            // Inject readaptation protocol based on absence duration
            this._injectReadaptationProtocol(dias);
            UI.renderDashboard();
        }
    },

    _injectReadaptationProtocol(diasAusente) {
        // Number of readaptation days depends on absence length
        // 2-3 días: 2 días readaptación (60% vol, RPE max 6)
        // 4-6 días: 3 días readaptación (50% vol, RPE max 6)
        // 7-9 días: 4 días readaptación (40% vol, RPE max 5)
        // 10+ días: 5 días readaptación (35% vol, RPE max 5)
        const diasReadapt = diasAusente <= 3 ? 2 : diasAusente <= 6 ? 3 : diasAusente <= 9 ? 4 : 5;
        const volPct      = diasAusente <= 3 ? 60 : diasAusente <= 6 ? 50 : diasAusente <= 9 ? 40 : 35;
        const rpeMax      = diasAusente <= 6 ? 6 : 5;

        // Save readaptation config to localStorage so plan adaptation can use it
        const uid = UserSystem.getCurrentUserId();
        const readaptConfig = {
        ...
        localStorage.setItem(`rabix_${uid}_readapt`, JSON.stringify(readaptConfig));

        // Inject overrides for readaptation days
        for (let i = 0; i < diasReadapt; i++) {
            const d = new Date(); d.setDate(d.getDate() + i);
            const dStr = `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;
            DB.saveOverride(dStr, 'READAPTACION');
        }

        const msg = `✅ Protocolo de retorno activado.
${diasReadapt} días de readaptación al ${volPct}% del volumen habitual.
RPE máximo: ${rpeMax}/10.

No compenses — el cuerpo recupera rápido con carga gradual.`;
    ...
    openEquipoModal(session) {
        const sd = typeof ESCENARIOS !== 'undefined' && ESCENARIOS[session?.escenario]
            ? ESCENARIOS[session.escenario]
            : { label: 'Entrenamiento Equipo', emoji: '🤝' };
        // Set modal title/emoji
        const titleEl = document.getElementById('equipo-modal-title');
        const emojiEl = document.getElementById('equipo-modal-emoji');
        if (titleEl) titleEl.textContent = sd.label;
        if (emojiEl) emojiEl.textContent = sd.emoji;
        // Reset sliders
        const rpeEl = document.getElementById('equipo-rpe');
        const minEl = document.getElementById('equipo-min');
        if (rpeEl) { rpeEl.value = 7; document.getElementById('equipo-rpe-display').textContent = 7;
            rpeEl.oninput = function() { document.getElementById('equipo-rpe-display').textContent = this.value; };
        }
        if (minEl) { minEl.value = 90; document.getElementById('equipo-min-display').textContent = 90; }
        document.getElementById('equipo-modal-overlay').classList.remove('hidden');
    },

    stepEquipoMin(delta) {
        const el = document.getElementById('equipo-min');
        if (!el) return;
        let val = parseInt(el.value || '90') + delta;
        val = Math.max(10, Math.min(180, val));
        el.value = val;
        document.getElementById('equipo-min-display').textContent = val;
    },

    confirmEquipo() {
        const minutos = parseInt(document.getElementById('equipo-min').value || '90');
        const rpe     = parseFloat(document.getElementById('equipo-rpe').value || '7');
        const carga   = SportModules.calcInternalLoad(rpe, minutos);
        DB.saveSet({
            cod: 0,
            fecha: Planner.todayStr(),
            ...
            vol_total: 0,
            carga_interna: carga,
            escenario: 'ENTRENAMIENTO_EQUIPO',
            notes: `Equipo: ${minutos} min — RPE ${rpe}`,
            sesion_num: 1,
            session_rpe: true,
        });
        document.getElementById('equipo-modal-overlay').classList.add('hidden');
        UI.renderDashboard();
    },
