        ...
        if (newUserForm) {
            newUserForm.addEventListener('submit', e => {
                ...
                try {
                    ...
                    // Marcar onboarding como completado con valores por defecto
                    // para que el usuario entre directo al dashboard
                    const users = UserSystem.getAllUsers();
                    const newUser = users.find(x => x.id === u.id);
                    if (newUser && !newUser.onboarding) {
                        newUser.onboarding = {
                            frecuenciaUlt7d: '3', disponibilidadDias: '3',
                            diaDescanso: '0', nivel: 'intermedio',
                            duracionSesion: '60', recuperacion: 'media',
                            diaPartido: '-1', equipamiento: 'completo',
                            historialLesiones: 'ninguna',
                            objetivo_composicion: 'ganar', temporada: 'libre',
                            experienciaDeporte: 'medio'
                        };
                        UserSystem.saveUsers(users);
                    }
                    App.initForUser(u.id);
                } catch (err) {
