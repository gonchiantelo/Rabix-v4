...
const UI = (() => {
    ...
    function buildExerciseCard(exData) {
        ...
        const recoSets  = (exData && typeof exData === 'object' && exData.series) ? exData.series : (ej.fatiga === 3 ? 4 : 4);
        const recoReps  = (exData && typeof exData === 'object' && exData.reps)   ? exData.reps   : (ej.fatiga === 3 ? '6-8' : ej.fatiga === 2 ? '10-12' : '15');
        const recoPause = (exData && typeof exData === 'object' && exData.pause)  ? exData.pause  : (ej.fatiga === 3 ? '180s' : ej.fatiga === 2 ? '90s' : '60s');
        ...
        card.innerHTML = `
      <div class="exercise-card-header" onclick="UI.openLogModal(${cod})" style="cursor:pointer;-webkit-tap-highlight-color:transparent;">
      ...
      ${hasRirBadge ? `<div class="suggestion-banner" style="margin:8px 12px 0;"><span class="suggestion-icon">🟢</span><span class="suggestion-text">RIR ≥ ${RIR_PROGRESSION_THRESHOLD} en última serie: subir carga</span></div>` : ''}
      ${hasLogs ? `<div class="ex-logged-sets">
        ...
        <button class="btn-add-set" onclick="UI.openLogModal(${cod})" style="-webkit-tap-highlight-color:transparent;">
    ...
    function initLogForm() {
        ...
        document.getElementById('log-form').addEventListener('submit', e => {
            ...
            DB.saveSet({ cod, fecha: fechaRegistro, peso, series, reps, rpe, rir, dolor, notes, vol_total: tonelaje, tonelaje, dolor_alert });
    ...
    function initGlobalListeners() {
        ...
        const objBadgeBtn = document.getElementById('objetivo-badge-static');
        if (objBadgeBtn) objBadgeBtn.addEventListener('click', openObjetivoModal);
